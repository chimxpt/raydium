...
public class VOptionScreen extends Screen {
    ...
    private VTextInputWidget createSearchField() {
        int rightMargin = 10;
        int padding = 10;
        // M8.128 (Raydium): кнопки «Support me» больше нет (вела на ko-fi апстрима; вернём со
        // своими ссылками) — поиск занимает всю верхнюю панель
    ...
    private void addButtonsWithSearchBar() {
        ...
        // M8.128 (Raydium): «Support me» удалена — вела на ko-fi апстрима VulkanMod;
        // вернём с собственными ссылками поддержки, когда они появятся.
        ...
        // M8.128 (Raydium): кнопка «доступно обновление» отключена — чекер сверяется с
        // Modrinth-страницей АПСТРИМА VulkanMod и для форка всегда бы врал. Вернём, когда
        // у Raydium появится собственная страница на Modrinth.
    }
    ...
    public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float delta) {
        ...
        // === RAYDIUM: ОБЯЗАТЕЛЬНАЯ атрибуция NVIDIA DLSS. NVIDIA RTX SDKs License, Supplement §7.1b:
        // приложение с DLSS SDK должно указывать использование SDK и марки NVIDIA в about-box/credits.
        // Экран настроек мода = наш about-box. Цвет — фирменный NVIDIA-зелёный (0x76B900). НЕ УДАЛЯТЬ. ===
        GuiRenderer.drawString(this.font,
                Component.literal("Powered by NVIDIA DLSS Ray Reconstruction"),
                8, this.height - 12, 0xFF76B900);
    }
