...
import net.vulkanmod.vulkan.rt.RtWorld;
...
public class DrawBuffers {
    ...
    public void upload(RenderSection section, UploadBuffer buffer, TerrainRenderType renderType) {
        ...
        // === RT PATCH (M8.25c): СЛОЙ БЕЗ ГЕОМЕТРИИ -> снять его BLAS (иначе ПРИЗРАК) ===
        // ⚠️ Настоящий путь призрака: игра зовёт upload с НЕпустым UploadBuffer, но НУЛЕВЫМ размером
        // (сломали последний стеклянный/слизевый блок — граней не осталось). Тогда doUpload=false,
        // хук постройки ниже не срабатывает, и старый BLAS остаётся в TLAS = призрак сломанного блока.
        // Прошлые фиксы (пустой uploadBuffer / сброс секции) сюда не доставали — буфер-то не null.
        // indexOnly-путь сюда НЕ доходит (вышел выше), поэтому валидную геометрию мы не тронем.
        if (DeviceManager.rayTracingSupported && RtWorld.INSTANCE != null && size == 0) {
            int sx = section.xOffset() >> 4, sy = section.yOffset() >> 4, sz = section.zOffset() >> 4;
            long gkey = (((long) sx & 0xFFFFFF) << 38) | (((long) sz & 0xFFFFFF) << 14)
                      | (((long) sy & 0x3FF) << 4) | (renderType.ordinal() & 0xF);
            RtWorld.INSTANCE.removeSection(gkey);
        }
        ...
        // === RT PATCH (M8.2/8.3/M8.5): построить BLAS этой секции ПОСЛЕ заливки её вершин на GPU ===
        // M8.5: TRANSLUCENT (вода/стекло) ТЕПЕРЬ трассируем — как «воду» (флаг water):
        // шейдер даёт Френель + отражение + преломление. TRIPWIRE по-прежнему пропускаем.
        if (DeviceManager.rayTracingSupported && RtWorld.INSTANCE != null && doUpload && size > 0
                && renderType != TerrainRenderType.TRIPWIRE) {
            // Только ставим в очередь (без GPU-работы) — батч соберётся раз в кадр в endFrame.
            int wx = section.xOffset(), wy = section.yOffset(), wz = section.zOffset();
            int sx = wx >> 4, sy = wy >> 4, sz = wz >> 4;
            // Ключ ВКЛЮЧАЕТ тип рендера — иначе SOLID/CUTOUT/TRANSLUCENT одной секции затирают друг друга.
            long key = (((long) sx & 0xFFFFFF) << 38) | (((long) sz & 0xFFFFFF) << 14)
                    | (((long) sy & 0x3FF) << 4) | (renderType.ordinal() & 0xF);
            boolean water = renderType == TerrainRenderType.TRANSLUCENT;
            RtWorld.INSTANCE.enqueueSection(key, wx, wy, wz, areaBuffer.getBufferId(), segment.offset,
                    size / vertexSize, vertexSize, renderType == TerrainRenderType.SOLID, water);
        }
        // === /RT PATCH ===
