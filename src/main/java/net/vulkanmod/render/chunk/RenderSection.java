...
//            Minecraft.getInstance().levelRenderer.updateGlobalBlockEntities(toRemove, toAdd);
    ...
    private void resetDrawParameters() {
        ...
        // === RT PATCH (M8.25a): СБРОС СЕКЦИИ -> снять ВСЕ её BLAS ===
        // Второй путь «призрака»: секция сбрасывается целиком (выгрузка/инвалидация), upload не
        // зовётся, и её BLAS остались бы в TLAS. Чистим все типы рендера этой секции.
        if (net.vulkanmod.vulkan.device.DeviceManager.rayTracingSupported
                && net.vulkanmod.vulkan.rt.RtWorld.INSTANCE != null) {
            int sx = this.xOffset >> 4, sy = this.yOffset >> 4, sz = this.zOffset >> 4;
            for (TerrainRenderType rt : TerrainRenderType.VALUES) {
                long key = (((long) sx & 0xFFFFFF) << 38) | (((long) sz & 0xFFFFFF) << 14)
                         | (((long) sy & 0x3FF) << 4) | (rt.ordinal() & 0xF);
                net.vulkanmod.vulkan.rt.RtWorld.INSTANCE.removeSection(key);
            }
            // === RT PATCH (M8.142): снять ЗАПЕЧЁННЫЙ свет секции (выгрузка чанка). ===
            net.vulkanmod.vulkan.rt.RtLights.removeSectionLights(
                    net.vulkanmod.vulkan.rt.RtLights.sectionKey(sx, sy, sz));
        }
