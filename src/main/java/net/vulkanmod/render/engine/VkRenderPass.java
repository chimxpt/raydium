...
//            this.encoder.getDevice().debugLabels().popDebugGroup();
    ...
    public void bindTexture(String string, @Nullable GpuTextureView gpuTextureView,
        ...
        // === RT PATCH (M8.14): текстура батча для захвата в RT ===
        // Партиклы (и прочее, что рисуется через проход рендера) биндят Sampler0 ЗДЕСЬ, а не через
        // VTextureSelector — тот для этого пути устаревший. Запоминаем её для RtEntities.collect*:
        // у частиц свой атлас, у партиклов ломания блоков — БЛОЧНЫЙ, поэтому нужна текстура
        // КАЖДОГО батча, а не одна общая.
        if (net.vulkanmod.vulkan.device.DeviceManager.rayTracingSupported
                && "Sampler0".equals(string) && gpuTextureView != null) {
            try {
                var gpuTex = (VkGpuTexture) gpuTextureView.texture();
                var glTex = net.vulkanmod.gl.VkGlTexture.getTexture(gpuTex.glId());
                if (glTex != null)
                    net.vulkanmod.vulkan.rt.RtEntities.setNextBatchTexture(glTex.getVulkanImage());
    ...
    public void drawIndexed(int vertexOffset, int firstIndex, int vertexCount, int instanceCount) {
        // === RT PATCH (M8.14): диапазон ОДНОГО СЛОЯ партиклов (текстуру уже поймал bindTexture) ===
        if (net.vulkanmod.vulkan.device.DeviceManager.rayTracingSupported)
            net.vulkanmod.vulkan.rt.RtEntities.collectParticleRange(vertexOffset, vertexCount);
        // === /RT PATCH ===
        if (this.closed) {
            throw new IllegalStateException("Can't use a closed render pass");
        } else {
