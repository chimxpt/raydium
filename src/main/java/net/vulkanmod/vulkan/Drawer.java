...
public class Drawer {
    ...
    public void draw(ByteBuffer vertexData, ByteBuffer indexData, VertexFormat.Mode mode, VertexFormat vertexFormat, int vertexCount) {
        // === RT PATCH (M8.4): вершины сущностей (NEW_ENTITY, QUADS) — в сбор для TLAS ===
        if (net.vulkanmod.vulkan.device.DeviceManager.rayTracingSupported
                && mode == VertexFormat.Mode.QUADS) {
            if (vertexFormat == com.mojang.blaze3d.vertex.DefaultVertexFormat.NEW_ENTITY)
                net.vulkanmod.vulkan.rt.RtEntities.collect(vertexData, vertexCount);
            // ПРЕДМЕТ В РУКЕ рисуется КАК БЛОК (32 Б) — его вершины мы раньше выбрасывали, и рука
            // держала пустоту. Перепаковываем в наш формат (см. RtEntities.collectBlock).
            else if (vertexFormat == com.mojang.blaze3d.vertex.DefaultVertexFormat.BLOCK)
                net.vulkanmod.vulkan.rt.RtEntities.collectBlock(vertexData, vertexCount,
                        net.vulkanmod.vulkan.rt.RtEntities.batchTexture());
            else
                net.vulkanmod.vulkan.rt.RtEntities.logSkipped(vertexFormat, vertexCount);   // ЗАМЕР: что ещё выбрасываем
        }
        // === /RT PATCH ===
