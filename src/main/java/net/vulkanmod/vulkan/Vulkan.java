...
//            Thread.dumpStack();
    ...
    /** === RT PATCH (M8.29): NGX (DLSS) требует сам VkInstance при инициализации. === */
    ...
    public static void cleanUp() {
        vkDeviceWaitIdle(DeviceManager.vkDevice);
        // === RT PATCH (M8.6): освободить RT-ресурсы (тысячи AS + буферы, сырой vkAllocateMemory)
        // ДО уничтожения устройства — иначе живые AS-объекты вешают teardown на NVIDIA (фриз выхода). ===
        net.vulkanmod.vulkan.rt.RtWorld.shutdown();
        net.vulkanmod.vulkan.rt.RtSnapshot.shutdown();
        net.vulkanmod.vulkan.rt.RtScreen.shutdown();
        // === /RT PATCH ===
...
//        debugCreateInfo.messageType(VK_DEBUG_UTILS_MESSAGE_TYPE_GENERAL_BIT_EXT | VK_DEBUG_UTILS_MESSAGE_TYPE_VALIDATION_BIT_EXT);
    ...
    private static void createVma() {
        try (MemoryStack stack = stackPush()) {
            ...
            // === RT PATCH (M8.2): буферам-входам BVH нужен device address ===
        ...
        // === RT PATCH (M8.2): самопроверка движка AS + инициализация BVH мира ===
        net.vulkanmod.vulkan.rt.RtSelfTest.run();
        net.vulkanmod.vulkan.rt.RtWorld.createInstance();
    }
