                ...
                switch (attribute.ioType) {
                    case "in" -> {
                        ...
                        if (vertAttribute != null) {
                            ...
                            // ⚠️ ВХОД БЕЗ ПАРЫ В ВЕРШИННОМ ШЕЙДЕРЕ (M8.67). Раньше объявление здесь
                            // просто ВЫБРАСЫВАЛОСЬ (return), а использования в коде оставались —
                            // компилятор падал с «undeclared identifier», и игра не запускалась
                            // ВООБЩЕ. Ловилось это на наборах ресурсов (Fresh Food, Fresh Music
                            // Discs), которые добавляют во фрагментный шейдер предметов свои
                            // переменные. В OpenGL непарный вход законен — он просто не заполнен;
                            // повторяем это: объявляем обычной переменной с нейтральным значением.
                            Initializer.LOGGER.warn(
                                    "[shader] {}: вход '{} {}' не отдаётся вершинным шейдером — объявляю "
                                            + "переменной по умолчанию (набор ресурсов подменил ФРАГМЕНТНЫЙ "
                                            + "шейдер, а вершинный к нему не подошёл)",
                                    this.pipelineName, type, id);
                            this.appendNode(new DefaultedInput(type, id));
                            return;
                        }
                    }
                    case "out" -> {
