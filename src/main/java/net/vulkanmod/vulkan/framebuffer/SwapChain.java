...
public class SwapChain extends Framebuffer {
    ...
    private int getPresentMode(IntBuffer availablePresentModes) {
        // === RT PATCH (M8.129): V-Sync без деления частоты пополам ===
        // Жёсткий FIFO (double-buffer VSync) при малейшем промахе кадра к развёртке ждёт ВСЮ
        // следующую — частота обрушивается вдвое (репорт: 135 -> 74). Mailbox (FastSync/
        // triple-buffer) тоже гасит разрывы, но опоздавший кадр НЕ делит частоту: FPS держится
        // у потолка монитора. Поэтому при включённом V-Sync предпочитаем Mailbox, а FIFO —
        // только фолбэк, если Mailbox драйвер не отдаёт.
        int requestedMode;
        if (vsync) {
            requestedMode = hasMode(availablePresentModes, VK_PRESENT_MODE_MAILBOX_KHR)
        ...
        for (int i = 0; i < availablePresentModes.capacity(); i++) {
            if (availablePresentModes.get(i) == requestedMode) {
                // === RT PATCH === Пишем ФАКТ, а не догадку: на Wayland режима Immediate может не
    ...
    /** M8.129: есть ли режим показа среди предложенных драйвером. */
    private static boolean hasMode(IntBuffer modes, int mode) {
        for (int i = 0; i < modes.capacity(); i++)
            if (modes.get(i) == mode) return true;
        return false;
    }

    /** Все режимы показа, которые предлагает драйвер под текущую оконную систему. */
    private String listModes(IntBuffer modes) {
        StringBuilder sb = new StringBuilder();
        for (int i = 0; i < modes.capacity(); i++) {
            if (i > 0) sb.append(", ");
            sb.append(getDisplayModeString(modes.get(i)));
        }
        return sb.toString();
    }
