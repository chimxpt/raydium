...
public class RtSnapshot {
    ...
    private void recordComputeImpl(VkCommandBuffer frameCmd, long tlasHandle, long vtxTableBuffer, VulkanImage atlasTex) {
        ...
        // M8.122e: без DLSS нет апскейлера — пониженный кадр растягивался бы на экран простым
        // билинейным блитом (мыло). Масштаб принудительно 100% (ручка в UI при этом погашена).
        if (!RtDlss.usable()) pct = 100;
