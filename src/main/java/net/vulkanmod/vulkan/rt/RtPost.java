...
public class RtPost {
    ...
    private static final String SRC = """
            ...
            void main() {
                ...
                // M8.159 УВЕЛИЧЕНИЕ БЕЗ DLSS. Раньше здесь стояла ТОЧЕЧНАЯ выборка (дробная часть
                // координаты просто отбрасывалась) — самый грубый способ, дающий лесенку. Из-за
                // неё масштаб трассировки приходилось держать на 100%, а это больше чем вдвое
                // дороже по лучам и убивало смысл встроенного денойзера: слабой видеокарте нужен
                // как раз пониженный масштаб. Теперь билинейная выборка (4 отсчёта) плюс лёгкое
                // подчёркивание контуров, которое возвращает резкость, съеденную интерполяцией.
                vec3 col;
                if (dlssOn) {
                    col = imageLoad(hdrIn, spix).rgb;
                } else {
                    vec2 sp = (vec2(pix) + 0.5) * srcSize / vec2(float(width), float(height)) - 0.5;
                    vec2 f  = fract(sp);
                    ivec2 b0 = ivec2(floor(sp));
                    ivec2 mx = ivec2(srcSize) - 1;
                    vec3 c00 = imageLoad(hdrIn, clamp(b0,               ivec2(0), mx)).rgb;
                    vec3 c10 = imageLoad(hdrIn, clamp(b0 + ivec2(1, 0), ivec2(0), mx)).rgb;
                    vec3 c01 = imageLoad(hdrIn, clamp(b0 + ivec2(0, 1), ivec2(0), mx)).rgb;
                    vec3 c11 = imageLoad(hdrIn, clamp(b0 + ivec2(1, 1), ivec2(0), mx)).rgb;
                    col = mix(mix(c00, c10, f.x), mix(c01, c11, f.x), f.y);
                    // подчёркивание: отклонение от среднего окрестности, мягко и без ореолов
                    vec3 avg = (c00 + c10 + c01 + c11) * 0.25;
                    col = max(col + (col - avg) * 0.6, vec3(0.0));
                }
                ...
                // ⚠️ Режим 7 («копилка денойзера») СЮДА НЕ ЗАХОДИТ: его рисует сам денойзер, и к
                // посту приходит уже готовая картинка. Без этого исключения он попал бы в ветку
                // ниже и показал бы шероховатость — последний else ловит всё подряд.
                if (dbg > 0 && dbg < 7 && !rasterIn) {
    ...
    // M8.158d: +1 режим — «копилка денойзера» (RtDenoise.DEBUG_VIEW). Он обрабатывается НЕ здесь,
    // а в самом денойзере: тот рисует счётчик накопления вместо кадра. Пост в этом режиме получает
    // уже готовую картинку и просто пропускает её дальше.
    public static final int DEBUG_MODES = 8;
